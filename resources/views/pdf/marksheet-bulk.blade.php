    ...
    @foreach($students as $student)
        @php
            $result = \App\Models\Result::where('exam_id', $exam->id)
                ...
                ->first();
            $breakdowns = $result ? $result->breakdowns()->with('subject')->get() : collect();
        @endphp
        ...
        @if($result)
            ...
            <table style="width:100%; border-collapse:collapse; margin-top:10px;">
                <tr>
                    ...
                    <td style="border:1px solid #000; padding:5px 8px; width:25%;">{{ $result->position ? $result->ordinal($result->position) : 'N/A' }}</td>
                </tr>
            </table>
        @else
            <p style="text-align:center; color:#999; margin-top:20px;">No result calculated for this student.</p>
        @endif
