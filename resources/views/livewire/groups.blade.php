<div>
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('academic_structure.groups') }}</h1>
                ...
                @forelse($groups as $group)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-right text-sm space-x-2">
                            <button wire:click="edit({{ $group->id }})" class="text-indigo-600 hover:text-indigo-800">{{ __('academic_structure.edit') }}</button>
                            <button wire:click="delete({{ $group->id }})" wire:confirm="{{ __('academic_structure.confirm_delete') }}" class="text-red-600 hover:text-red-800">{{ __('academic_structure.delete') }}</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="2" class="px-6 py-8 text-center text-sm text-gray-500">{{ __('academic_structure.no_records') }}</td></tr>
                @endforelse
            </tbody>
        </table>
        </div>
        <div class="px-6 py-3 border-t border-gray-200">{{ $groups->links() }}</div>
    </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4" x-on:click.self="$wire.set('showModal', false)">
                <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6" x-on:click.stop>
                    ...
                    <form wire:submit="save" class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('academic_structure.group_name') }} *</label>
